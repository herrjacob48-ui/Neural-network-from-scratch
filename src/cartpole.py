"""
Cart-Pole Balancing — another environment for your Network to learn, same trick
as the ball game: your Network only does supervised learning (train(x, y) with a
known target), and there's no natural "correct answer" for control problems like
this... UNLESS you already have a controller that works. So:

  1. A small hand-tuned PD controller (a few lines of math, tuned below) can
     already balance the pole reliably.
  2. We ask that controller "what force would you apply in this state?" for a
     TRAINING state, and use that as the target y for input x = [cart_x,
     cart_x_dot, pole_theta, pole_theta_dot]. That's the (x, y) pair fed to
     net.train().
  3. Separately, the network's own live prediction drives the pole you actually
     see on screen, so you watch it go from random twitching to holding the
     pole up as it imitates the PD controller.

IMPORTANT DESIGN NOTE — why training states are sampled randomly, not taken
from the on-screen pole's current position:
A controller (whether the PD one or a partially-trained network) mostly keeps
the pole hovering near upright-and-centered. If you only trained on whatever
state the visible pole happens to be in, the network would see almost nothing
but "near equilibrium" states for most of its training time and would quietly
forget how to correct large errors (it never has a reason to practice on
those). It would look great for a few seconds then faceplant on the first big
random disturbance. So training samples are drawn uniformly across the whole
state space every step, guaranteeing constant practice on hard states too,
while the on-screen pole is a separate, cosmetic rollout of "the network
controlling for real" so you can watch progress. This is a well-known pitfall
in imitation learning (covariate shift) and this is the standard fix.

Physics model is the classic (nonlinear) cart-pole equations, same ones used in
OpenAI Gym's CartPole environment, integrated with simple Euler steps.

CONTROLS:
  T         - toggle training on/off
  H         - toggle control: NN vs. the PD controller (for comparison)
  R         - reset the network (re-randomize weights) and game stats
  SPACE     - hold to fast-forward (extra train-only physics steps per frame)
  ESC / Q   - quit
